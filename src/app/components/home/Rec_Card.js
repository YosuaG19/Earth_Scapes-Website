        ...
        <div id="rec_card" className="w-[30%] min-w-[30%] h-full bg-white relative flex items-end rounded-tl-[2rem] rounded-br-[1rem] overflow-hidden shadow-lg/30">
            <div className="absolute w-full h-full top-0 left-0 z-[2]">
                <div className="relative w-full h-full ">
                    ...
                    <Image id="img" fill className="z-[0] absolute h-full" src={props.img}></Image>
                </div>
            </div>

            <div className="realtive flex h-[30%] w-full bg-black/30 p-[.5rem] z-[3]">
                <div className="text-white">
                    <h2 className="abo text-[35px] font-bold">{props.name},</h2>
                    <p className="abo text-[15px] max-w-[100%]">{props.loc}</p>
                </div>
                <div className="absolute right-[1rem] top-[.5rem] arti text-[30px] text-white">{props.rate}</div>
                <button className="absolute bottom-4 right-4 p-[.5rem] bg-white rounded-lg">Look up</button>
            </div>
        </div>
    )
}
