...
function Rec_Card(props) {
    return (
        ...
        <div id="rec_card" className="group w-[25vw] min-w-[300px] h-full bg-[#e8e8da] relative flex items-end rounded-tl-[2rem] rounded-br-[1rem] overflow-hidden shadow-lg cursor-pointer">
            ...
            {/* TEXT CONTENT */}
            <div className="relative flex w-full items-end p-[1.5rem] z-[3]">
                <div className="text-[#e8e8da]">
                    <h2 className="abo text-[25px] font-bold leading-tight">{props.name},</h2>
                    <p className="abo text-[15px] max-w-[100%] opacity-80">{props.loc}</p>
                </div>
                <div className="absolute right-[1.5rem] bottom-[1.5rem] arti text-[30px] text-[#e8e8da] font-light">
                    {props.rate}
                </div>
            </div>
        </div>
